...
@dataclass
class Episode:
    observations: torch.Tensor  # Changed from ByteTensor for CartPole (float observations)
    actions: torch.LongTensor
    rewards: torch.FloatTensor
    ends: torch.LongTensor  # 1 when episode ends, 0 otherwise
    ...
    def merge(self, other: Episode) -> Episode:
        """Merge two episodes (used for extending episodes)"""
        return Episode(
